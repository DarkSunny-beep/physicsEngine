# Physics Engine

# OverView
The majority of the core engine code in this repository is based on Ian Millington's book, *Game Physics Engine Development*.  
However, it also includes various custom programs and demonstrations where I have personally refactored the code, optimized structures, and added new features.  

  
In this README, I will focus only on explaining the core code and providing any noteworthy additional details.

# Breakable Bridge Demo

1. basic bridge Demo

<video src="./demoMov/bridgeDemo.mov" controls width="100%"></video>


2. breakable Bridge Demo (+implemented destruction logic)
<video src="./demoMov/breakableBridgeDemo.mov" controls width="100%"></video>
https://github.com/user-attachments/assets/e36ad5cf-1dec-4036-86b7-98e6bcddc3a0

** You can read more details on this website. (Notion)  
https://glib-spell-c41.notion.site/breakableBridgeDemo-3b955979569c805f9ce5f52345aa2773?pvs=143

